...
# move channels to be first using moveaxis: channels x height x width, needed for pytorch
...
model = resnet18(weights=weights) # print(model) prints all layers and configs

# change the clf head
num_features = model.fc.in_features
model.fc = nn.Linear(in_features=num_features, out_features=3) # 3 cls
...
for epoch in tqdm(range(n_epochs)):
    ...
    y_pred = torch.nn.functional.softmax(logits, dim=1).argmax()...
    ...
    # pca feature space
    ax["b"].scatter(X_extracted_plot[:, 0], X_extracted_plot[:, 1], c=y_test)
