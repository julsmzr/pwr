...
class CombinedAL(ClassifierMixin, BaseEstimator):
    def __init__(self, base_estimator: BaseEstimator = GaussianNB(), budget: float = 0.01, budget_split_ratio: float = 0.05, random_state: int = None):
        self.base_estimator = base_estimator
        self.budget = budget
        self.budget_split_ratio = budget_split_ratio
    ...
    def fit(self, X_seed, y_seed, X_unlabeled, y_unlabeled):
        ...
        self.n_queries_border = int(self.n_queries * self.budget_split_ratio)
        self.n_queries_random = self.n_queries - self.n_queries_border

        probas = self.clf_.predict_proba(X_unlabeled)
        self.distances = np.argsort(
            np.abs(probas[:, 0] - 0.5)
            )[:self.n_queries_border]
        ...
        self.indices = self.random.choice(
            np.delete(np.arange(X_unlabeled.shape[0]), self.distances), 
            size=self.n_queries_random, 
            replace=False
            )
